import pandas
import matplotlib.pyplot as plt
levels = ['legacy', 'partial', 'element', 'full']
orders = range(1,9)
devices = ['cpu', 'cuda']
device = 'ceed-cuda'
min_dof = 1e3
max_dof = 1e8
min_mdofs = 1e-3
max_mdofs = 5000
min_mdofs_asm = 1e-3
max_mdofs_asm = 10
min_time = 1e-4
max_time = 100 #30*60
######################
# CG (apply) analysis

plt.figure()

if True:
    data = pandas.read_json("lassen/bench.json")
    for order in orders:
        case = data.loc[(data['order'] == order) & (data['assembly'] == 'partial') & (data['device'] == device)].sort_values('ndofs')
        plt.semilogx(case['cg_time_per_iter'], case['cg_mdofs_per_sec'], marker='+', label=order)
else:
    from postprocess_base import read_logs
    data = read_logs(["noether-64/petsc-bps-2020-07-01.0.txt"])
    for degree in orders:
        case = data.loc[(data['degree'] == degree) & (data['bp'] == "3")].sort_values('num_unknowns')
        plt.semilogx(case['time_per_it'], case['cg_iteration_dps']*1e-6, marker='+', label=degree)


#plt.ylim( [min_mdofs, max_mdofs] )
#plt.xlim( [min_dof, max_dof] )
plt.grid(True)
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)

## Show the plots...
#plt.show()
#import matplotlib2tikz

#matplotlib2tikz.save("jed.tex")
import tikzplotlib

tikzplotlib.save("jed.tex")
