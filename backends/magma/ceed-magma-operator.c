...
static int CeedOperatorSetup_Magma(CeedOperator op) {
  ...
  // Infields
  ierr = CeedOperatorSetupFields_Magma(qf, op, 0,
                                      impl->evecs, impl->qvecsin, 0,
                                      numinputfields, Q, numelements);
  CeedChk(ierr);

  // Outfields
  ierr = CeedOperatorSetupFields_Magma(qf, op, 1,
...
//------------------------------------------------------------------------------
static inline int CeedOperatorSetupInputs_Magma(CeedInt numinputfields,
    CeedQFunctionField *qfinputfields, CeedOperatorField *opinputfields,
    CeedVector invec, const bool skipactive, CeedOperator_Magma *impl,
    CeedRequest *request) {
  CeedInt ierr;
  CeedEvalMode emode;
  CeedVector vec;
...
// Input Basis Action
//------------------------------------------------------------------------------
static inline int CeedOperatorInputBasis_Magma(CeedInt numelements,
    CeedQFunctionField *qfinputfields, CeedOperatorField *opinputfields,
    CeedInt numinputfields, const bool skipactive, CeedOperator_Magma *impl) {
  CeedInt ierr;
...
//------------------------------------------------------------------------------
static inline int CeedOperatorRestoreInputs_Magma(CeedInt numinputfields,
    CeedQFunctionField *qfinputfields, CeedOperatorField *opinputfields,
    const bool skipactive, CeedOperator_Magma *impl) {
  CeedInt ierr;
  CeedEvalMode emode;
  CeedVector vec;
...
static int CeedOperatorApplyAdd_Magma(CeedOperator op, CeedVector invec,
  ...
  ierr = CeedOperatorSetupInputs_Magma(numinputfields, qfinputfields,
                                      opinputfields, invec, false, impl,
                                      request); CeedChk(ierr);

  // Input basis apply if needed
  ierr = CeedOperatorInputBasis_Magma(numelements, qfinputfields, opinputfields,
  ...
  ierr = CeedOperatorRestoreInputs_Magma(numinputfields, qfinputfields,
...
//------------------------------------------------------------------------------
static int CeedOperatorLinearAssembleQFunction_Magma(CeedOperator op,
...
//------------------------------------------------------------------------------
extern "C" __device__ void CeedOperatorGetBasisPointer_Magma(const CeedScalar **basisptr,
  ...
  for (CeedInt e = blockIdx.x*blockDim.z + threadIdx.z; e < nelem;
    ...
    for (CeedInt eout = 0; eout < NUMEMODEOUT; eout++) {
      ...
      if (emodeout[eout] == CEED_EVAL_GRAD)
        dout += 1;
      CeedOperatorGetBasisPointer_Magma(&bt, emodeout[eout], identity, interpout,
      ...
      for (CeedInt ein = 0; ein < NUMEMODEIN; ein++) {
        ...
        if (emodein[ein] == CEED_EVAL_GRAD)
          din += 1;
        CeedOperatorGetBasisPointer_Magma(&b, emodein[ein], identity, interpin,
...
//------------------------------------------------------------------------------
static inline int CeedOperatorAssembleDiagonalSetup_Magma(CeedOperator op,
  ...
  CeedOperatorDiag_Magma *diag = impl->diag;
...
//------------------------------------------------------------------------------
static inline int CeedOperatorAssembleDiagonalCore_Magma(CeedOperator op,
  ...
  // Setup
  if (!impl->diag) {
    ierr = CeedOperatorAssembleDiagonalSetup_Magma(op, pointBlock); CeedChk(ierr);
  }
  CeedOperatorDiag_Magma *diag = impl->diag;
  assert(diag != NULL);
...
//------------------------------------------------------------------------------
static inline int CeedOperatorLinearAssembleDiagonalCompositeCore_Magma(
  ...
  for (CeedInt i = 0; i < numSub; i++) {
    ierr = CeedOperatorAssembleDiagonalCore_Magma(subOperators[i], assembled,
...
//------------------------------------------------------------------------------
static int CeedOperatorLinearAssembleDiagonal_Magma(CeedOperator op,
    CeedVector assembled, CeedRequest *request) {
  int ierr;
  bool isComposite;
  ierr = CeedOperatorIsComposite(op, &isComposite); CeedChk(ierr);
  if (isComposite) {
    return CeedOperatorLinearAssembleDiagonalCompositeCore_Magma(op, assembled,
           request, false);
  } else {
    return CeedOperatorAssembleDiagonalCore_Magma(op, assembled, request, false);
  }
}
...
//------------------------------------------------------------------------------
static int CeedOperatorLinearAssemblePointBlockDiagonal_Magma(CeedOperator op,
    CeedVector assembled, CeedRequest *request) {
  int ierr;
  bool isComposite;
  ierr = CeedOperatorIsComposite(op, &isComposite); CeedChk(ierr);
  if (isComposite) {
    ...
    return CeedOperatorAssembleDiagonalCore_Magma(op, assembled, request, true);
  }
}
...
//------------------------------------------------------------------------------
static int CeedOperatorCreateFDMElementInverse_Magma(CeedOperator op) {
  int ierr;
  Ceed ceed;
  ierr = CeedOperatorGetCeed(op, &ceed); CeedChk(ierr);
